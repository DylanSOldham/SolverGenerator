...
const double end_time = std::pow(10, 2);
const double sample_interval = std::pow(10, 0);
...
const double maximum_num_steps = 5000;
...
double __summation_5(double* values) {
	double sum = 0.0;
	for (size_t i = 3; i < MAX_CLUSTER_SIZE; i++) {
		sum += ((alpha_vv((i))) * (values[INDEX_Cv_START + ((i) - 1)]));
	}
	return sum;
}
...
double b_v(double n, double* values)
{
	return ((((((beta_vi((n))) * (values[INDEX_Ci_START + ((1) - 1)]))) + (((beta_vv((n))) * (values[INDEX_Cv_START + ((1) - 1)]))))) + (alpha_vv((n))));
}
...
double i_bias_factor(double n)
{
	return ((i_dislocation_bias) + (((((((std::sqrt(((burgers_vector) / (((((8) * (PI))) * (lattice_param)))))) * (i_loop_bias))) - (i_dislocation_bias))) * (((1) / (std::pow(n, ((i_dislocation_bias_param) / (2)))))))));
}

double v_bias_factor(double n)
{
	return ((v_dislocation_bias) + (((((((std::sqrt(((burgers_vector) / (((((8) * (PI))) * (lattice_param)))))) * (v_loop_bias))) - (v_dislocation_bias))) * (((1) / (std::pow(n, ((v_dislocation_bias_param) / (2)))))))));
}

double i_binding_energy(double n)
{
	return ((i_formation) + (((((((i_binding) - (i_formation))) / (((std::pow(2, 0.8)) - (1))))) * (((std::pow(n, 0.8)) - (std::pow(((n) - (1)), 0.8)))))));
}

double v_binding_energy(double n)
{
	return ((v_formation) + (((((((v_binding) - (v_formation))) / (((std::pow(2, 0.8)) - (1))))) * (((std::pow(n, 0.8)) - (std::pow(((n) - (1)), 0.8)))))));
}

double r(double n)
{
	return std::sqrt(((((((std::sqrt(3)) * (std::pow(lattice_param, 2)))) * (n))) / (((4) * (PI)))));
}
...
void get_initial_state(N_Vector state) {
	...
	for (size_t n = 1; n <= MAX_CLUSTER_SIZE; ++n)
	{
		values[INDEX_Ci_START + ((n) - 1)] = ((1) * (std::pow(10, -(1))));
	}

	for (size_t n = 1; n <= MAX_CLUSTER_SIZE; ++n)
	{
		values[INDEX_Cv_START + ((n) - 1)] = ((1) * (std::pow(10, -(1))));
	}
